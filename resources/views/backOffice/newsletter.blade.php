                                ...
                                <table class="table table-striped">
                                    ...
                                    @foreach($inscriptions as $inscription)
                                        <tr>
                                            ...
                                            <td>
                                                <label for=""
                                                       class="state badge {{$inscription->state == 1 ? "badge-success" : "badge-danger" }}">{{$inscription->state == 1 ? "Activé" : "Bloqué" }}</label>
                                            </td>
                                            <td>
                                                {{$inscription->created_at}}
                                            </td>
                                            <td>
                                                <form style="display: inline;" class="change-state-form" method="POST"
                                                      action="{{ route('admin.managers.changeRole')}}">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{$inscription->id}}">
                                                    <input type="hidden" class="state" name="state"
                                                           value="{{$inscription->state == 1 ? 0 : 1}}">
                                                    <button type="submit"
                                                            class="btn badge badge-success change-state">{{$inscription->state == 1 ? "Bloquer" : "Activer"}}</button>
                                                </form>
                                                <a href=""></a>
                                            </td>
                                        </tr>
