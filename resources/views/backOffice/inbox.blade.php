                                    ...
                                    <div class="col-md-6 search-form">
                                        <form action="#" class="text-right">
                                            <div class="input-group">
                                                <input type="text" class="form-control input-sm search-msg"
                                                       placeholder="Search">
                                                <span class="input-group-btn">
                                            <button type="submit" name="search"
                                                    class="btn_ btn-primary btn-sm search"><i class="fa fa-search"></i></button></span>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        @foreach($messages as $message)
                                            <tr {{$message->state == 1 ? 'class=read' : ""}} class="show-msg"
                                                data-toggle="modal" data-target="#compose-modal{{$message->id}}"
                                                data-id="{{$message->id}}">
                                                <td class="action"><i
                                                            class=" {{$message->state == 1 ? 'far fa-envelope-open' : 'fa fa-envelope'}}"></i>
                                                </td>
                                                <td class="name"><a>{{$message->full_name}}</a></td>
                                                <td class="subject"><a>{{$message->subject}} </a></td>
                                                <td class="time">{{$message->created_at}}</td>
                                                <td class="action delete-msg" data-id="{{$message->id}}"><i
                                                            class="fas fa-trash-alt"></i></td>
                                            </tr>
                                            <!-- BEGIN COMPOSE MESSAGE -->
                                            <div class="modal fade" id="compose-modal{{$message->id}}" tabindex="-1"
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header bg-blue">
                                                                ...
                                                                <h4 class="modal-title"><i class="fa fa-envelope"></i>
                                                                    Compose New Message</h4>
                                                            </div>
                                                            <div class="modal-body">
                                                                <span class="from title">From </span> : <span
                                                                        class="user content">{{$message->full_name}}</span><br>
                                                                <span class="about title">About </span> : <span
                                                                        class="subject content">{{$message->subject}}</span><br>
                                                                <hr>
